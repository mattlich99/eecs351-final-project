<template>
  <div class="text-justify">
    <p>
      ...
      creating our algorithm we have discovered solutions to problems we didn’t
      ...
      a few spikes. However, as displayed in the previous sections, this was not
      the case, as we had to additionally implement the low pass filter to
      actually detect the snare and kick drum, and couldn't even use the cross
      correlation algorithm because of the existence of other instruments in the
      same track. However, we were surprised to find that even without this
      additional signal processing algorithm, we were able to obtain such
      accurate results considering the difficulty of the problem. Even so, more
      can be done to improve the accuracy and speed of the algorithm.
    </p>
    <p>
      To improve the accuracy of our algorithm, we could split the unknown audio
      up into different instruments first and then try to identify the split up
      instruments. This would allow us to effectively use the cross correlation
      algorithm. Another way to improve the accuracy of the algorithm would be
      to apply the signal envelope to the hi hat drum kit component. The signal
      envelope would capture the shape of the hi-hat more accurately than the
      average filter, and could even be used on the other instruments as well.
      Finally, we could continue to improve our accuracy in the following way:
      for each drum component, instead of comparing just that component's
